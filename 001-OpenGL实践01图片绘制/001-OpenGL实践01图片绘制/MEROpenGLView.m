...
@interface MEROpenGLView () <GLKViewDelegate>
...
- (void)countImageSize {
    CGFloat screenRatio = CGRectGetHeight([UIScreen mainScreen].bounds) / CGRectGetWidth([UIScreen mainScreen].bounds);
    ...
    if (self.type == MEROpenGLViewTypeLandscape) {
        screenRatio = 1 / screenRatio;
    }
...
- (void)uploadTexture {
    ...
    GLKTextureInfo *textureInfo = [GLKTextureLoader textureWithContentsOfData:imageData options:options error:NULL];
...
- (UIImage *)image {
    if (!_image) {
        NSString *imageName = (_type == MEROpenGLViewTypePortrait) ? @"sakura_bicycle" : @"sakura_lay";
